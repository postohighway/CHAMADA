...
function computeTargetsFromRotacao() {
  // MODO MANUAL: rotacao.last_medium_id é o PRÓXIMO a ser chamado (não é "último" da fila).
  const byId = new Map((mediumsAll || []).map((m) => [m.id, m]));

  const nextMesaDir = rotacao?.mesa_dirigente ? (byId.get(rotacao.mesa_dirigente) || null) : null;
  const nextMesaInc = rotacao?.mesa_incorporacao ? (byId.get(rotacao.mesa_incorporacao) || null) : null;
  const nextMesaDes = rotacao?.mesa_desenvolvimento ? (byId.get(rotacao.mesa_desenvolvimento) || null) : null;
  const nextPsico   = rotacao?.psicografia ? (byId.get(rotacao.psicografia) || null) : null;
...
async function persistRotacaoFromClicks() {
  // MODO MANUAL:
  // Você marca (no fim da chamada) quem será o PRÓXIMO:
  // - Dirigente Mesa  -> status "M" (apenas 1)
  // - Psicografia     -> status "PS" (apenas 1)
  // - Incorporação    -> status "M" (apenas 1)
  // - Desenvolvimento -> status "M" (apenas 1)
  const active = (mediumsAll || []).filter((m) => m.active === true);

  function pickOne(group, status) {
    const st = (status || "").toUpperCase();
    const found = active.find((m) => m.group_type === group && (chamadasMap.get(m.id) || "").toUpperCase() === st);
    return found ? found.id : null;
  }

  let nextMesaDir = pickOne("dirigente", "M");
  let nextPsico   = pickOne("dirigente", "PS");
  let nextMesaInc = pickOne("incorporacao", "M");
  let nextMesaDes = pickOne("desenvolvimento", "M");

  // Não permitir mesma pessoa em Mesa e Psicografia
  if (nextMesaDir && nextPsico && nextMesaDir === nextPsico) {
    // Prioriza Mesa e zera Psicografia (você escolhe outro)
    nextPsico = null;
  }

  // Grava no banco: rotacao.last_medium_id passa a significar "próximo"
  if (nextMesaDir !== null) await sbPatch(`rotacao?group_type=eq.mesa_dirigente`, { last_medium_id: nextMesaDir });
  if (nextMesaInc !== null) await sbPatch(`rotacao?group_type=eq.mesa_incorporacao`, { last_medium_id: nextMesaInc });
  if (nextMesaDes !== null) await sbPatch(`rotacao?group_type=eq.mesa_desenvolvimento`, { last_medium_id: nextMesaDes });

  // Psicografia pode ficar null (se você não marcou ou se bateu com Mesa)
  await sbPatch(`rotacao?group_type=eq.psicografia`, { last_medium_id: nextPsico });
}
...
(async function init() {
  ...
  function clearSameGroupStatus(targetStatus, keepId) {
    const st = (targetStatus || "").toUpperCase();
    if (st !== "M" && st !== "PS") return;

    const group = m.group_type;
    const active = (mediumsAll || []).filter((x) => x.active === true && x.group_type === group);

    for (const x of active) {
      if (x.id === keepId) continue;
      const cur = (chamadasMap.get(x.id) || "").toUpperCase();
      if (cur === st) {
        chamadasMap.set(x.id, "");
      }
    }
  }
